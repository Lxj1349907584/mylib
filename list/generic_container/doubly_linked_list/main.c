...
int StuMsgDataCmp(void *data1, void *data2)
{
	...
	if (strcmp(stuMsg1->sex, stuMsg2->sex) > 0) {
		return 1;
	} else if (strcmp(stuMsg1->sex, stuMsg2->sex) == 0) {
		return 0;
	} else {
		return -1;
	}
...
test1()
{
	...
	RemoveNode(&StuMsgLinkList, (void*)&stu1);
	RemoveNode(&StuMsgLinkList, (void*)&stu2);
	RemoveNode(&FigureMsgLinkList, (void*)&fig2);
	ShowList(&StuMsgLinkList);
	ShowList(&FigureMsgLinkList);
}
...
{	
	...
	printf("PopBack_delete:\n");
	GetBack(&StuMsgLinkList, (void*)&stuMsg);
	StuMsgLinkList.ops.dataDisplay((void*)&stuMsg);
	PopBack(&StuMsgLinkList);
	printf("in list:\n");
	ShowList(&StuMsgLinkList);
